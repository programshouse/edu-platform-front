...
export function ProfileHero({ student }: ProfileHeroProps) {
  ...
  // Safe values in case the API returns undefined/null
  ...
  const courses = student?.stats?.courses ?? 0;
  const points = student?.stats?.points ?? 0;
  const rank = student?.stats?.rank ?? 0;
    ...
    <>
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-14">
              ...
              <div className="flex flex-col sm:flex-row flex-wrap items-center sm:items-start gap-x-5 gap-y-1 mt-2">
                ...
                {registeredAt && (
                  <span className="flex items-center gap-1.5 text-blue-100/80 text-sm">
                    ...
                    {t("hero.memberSince")} {registeredAt}
                  </span>
                )}
              </div>
            </motion.div>
          ...
          >
            {stats.map(
              ({ icon: Icon, value, label, color, bg, ring }) => (
                <div
                  key={label}
                ...
                >
                  <div
                    className={`w-9 h-9 rounded-xl ${bg} ${ring} ring-1 flex items-center justify-center shrink-0`}
                  >
